...
#define USER_PASSWORD FIREBASE_USER_PASS
...
void setup()
{
    ...
    Database.setSSEFilters("keep-alive");
    Database.get(aClient, "/test/stream", asyncCB, true, "streamTask");
}
    ...
    if (ssl_client.connect(host.c_str(), 443) > 0)
    {
        ...
        if (ssl_client.print(header) == header.length())
        {
            if (ssl_client.print(payload) == payload.length())
            {
                ...
                ms = millis();
                while (ssl_client.connected() && millis() - ms < 5000)
                {
